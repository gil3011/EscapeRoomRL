...
Per your steer, Room 1's board is **fixed and hand-designed**, not regenerated from
`random_layout()`. Defined in a new `engine/boards.py` as plain constants plus a
`make_room1_grid(slip_prob, goal_reward, trap_reward) -> GridWorld` factory — the *structure*
(which cells are what) is frozen; the *parameters* (rewards, slip probability, γ) stay fully
tunable from the sidebar.
...
- **Walls/barriers**: 13 cells in 5 small clusters scattered across the board, not one straight
  ...
  start, and the open cells form one single connected region with no isolated pockets — before
  freezing it as a static constant. Moving into a wall, or off the edge of the board, is illegal
  — the agent stays put (already enforced by `GridWorld._move`, unchanged from Sprint 1).
- **Traps** (terminal, negative reward): `{(4,6), (7,3), (2,8)}`.
- **Slippery cells**: `{(1,1),(1,7),(3,2),(3,8),(5,1),(5,8),(6,4),(7,6),(8,1),(8,7)}` — 10 cells
  scattered through the open areas.
- **Goal** (terminal, high reward): `(9,9)`.

(Visualized for you earlier in this conversation — gray = wall, blue = slippery, red = trap,
green = goal, purple = start.)

### Reward model — no step reward
Per your correction: the Bellman equation doesn't need a separate "step reward" bolted on top —
`R(s,a,s')` is 0 for every non-terminal transition; the only nonzero rewards are `goal_reward`
(entering the goal) and `trap_reward` (entering a trap). This is already exactly how
`GridWorld._reward_for` works from Sprint 1, just with `step_reward` fixed at `0.0` instead of a
small negative default. With `goal_reward` high and γ<1, V(s) still naturally encodes "closer to
the goal is better" purely from discounting — roughly `V(s) ≈ γ^d · goal_reward` for a cell `d`
steps from the goal under the optimal policy — no artificial per-step cost needed to make the
agent want to hurry. A lower γ makes V decay toward 0 away from the goal *faster*, which is
exactly the lever for showing that visually on the heatmap; a high `goal_reward` keeps that decay
from flattening the whole board into indistinguishable near-zero values. `step_reward` is
dropped from Room 1's sidebar entirely (fixed internally at `0.0`) — `GridWorld`'s own default
stays available for any later room that wants a step cost.
...
- `expected_steps_to_absorption(model, states, policy_dist, theta, max_iterations)` — **new**,
  ...
  expected step count, not a discounted return). Named "to absorption" rather than "to goal"
  because with real traps on the board, a random policy's episode can end at a trap instead of
  the goal — this function measures expected time to *either*. Used twice: once with a
  uniform-random policy (→ the scoring baseline below) and once with the learned greedy policy
  (→ an "optimal expected steps" metric shown next to it; a good policy should reach the *goal*
  specifically almost every time, so this number stays meaningful).
...
well-trained run score near 0. With only 3 trap cells out of ~88 free cells, a random walker
mostly wanders the open board rather than immediately finding a trap, so this should still land
on a sizeable number (tens to low hundreds of steps) — computed once at startup (the board is
static, so this never needs recomputing), shown in the sidebar as a reference ("Par — random
baseline: ~N steps"). The same convention carries over to Rooms 2-4 later.

A "Run escape attempt" click does **one** stochastic rollout (real slip applies) of the final
greedy policy, capped at `max_attempt_steps`. `success` = reached the goal specifically (a
trap or timeout is a failure, scoring 0 regardless of steps). Score =
...
**Environment** (board structure is fixed; these tune its behavior, not its layout)
- `slip_prob` — slider 0.0–0.9, default **0.2**
- `goal_reward` — slider 1.0–100.0, default **20.0** — kept high on purpose (see Reward model
  above): with no step cost, this plus γ is the *only* thing shaping V(s) across the board
- `trap_reward` — slider -100.0–-1.0, default **-20.0**
- (no `step_reward` — see Reward model above)
...
- [x] `engine/boards.py` — `ROOM1_WALLS`/`ROOM1_TRAPS`/`ROOM1_SLIPPERY`/`ROOM1_START`/`ROOM1_GOAL`
  constants + `make_room1_grid(slip_prob, goal_reward, trap_reward)` (`step_reward` fixed at 0.0
  internally, not a parameter — see Reward model above)
- [x] `tests/test_boards.py` — the goal is reachable from the start without passing through a
  wall (BFS, treating traps as passable-but-undesirable); none of walls/traps/slippery/start/goal
  overlap (easy mistake when hand-placing ~22 coordinates)
...
- [x] `tests/test_dp_solver.py` — converges to a hand-checked answer on a tiny grid; with
  ...
  goal along a clear path (confirms discounting alone is doing the work); sanity check on
  `expected_steps_to_absorption` (e.g. a trivial 1-step grid returns 1; a grid where the policy
  walks straight into a trap returns a small number)
